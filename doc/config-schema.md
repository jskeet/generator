...
| `issue_tracker_override` | string | Allows the issue_tracker field in .repo-metadata.json to be overridden. This should only be used for packages which don't include generated code. Anything generated from an API should use the issue tracker from the service config or the librarian sdk.yaml file. |
